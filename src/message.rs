    ...
    T: MessageField,
{
    ...
    fn encode_raw(&self, buf: &mut impl BufMut) {
        if !self.is_empty() {
            crate::encoding::message::encode_repeated(1, self, buf);
        }
    }

    fn merge_field(&mut self, tag: u32, wire_type: WireType, buf: &mut impl Buf, ctx: DecodeContext) -> Result<(), DecodeError> {
        if tag == 1 {
            crate::encoding::message::merge_repeated(wire_type, self, buf, ctx)
        } else {
    ...
    fn encoded_len(&self) -> usize {
        if self.is_empty() { 0 } else { crate::encoding::message::encoded_len_repeated(1, self) }
    }
