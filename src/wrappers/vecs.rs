...
impl ProtoWire for VecDeque<u8> {
    type EncodeInput<'a> = &'a VecDeque<u8>;
    const KIND: ProtoKind = ProtoKind::Bytes;

    #[inline(always)]
    fn encoded_len_impl(value: &Self::EncodeInput<'_>) -> usize {
        if value.is_empty() { 0 } else { unsafe { Self::encoded_len_impl_raw(value) } }
    }
    ...
    fn encode_raw_unchecked(value: Self::EncodeInput<'_>, buf: &mut impl BufMut) {
        let (left, right) = value.as_slices();
        buf.put_slice(left);
        buf.put_slice(right);
    }
