...
pub trait ProtoExt: Sized {
    ...
    {
        message::encode_repeated::<Self, I>(tag, values, buf);
    }
    ...
    fn merge_repeated_field(wire_type: WireType, values: &mut Vec<Self::Shadow<'_>>, buf: &mut impl Buf, ctx: DecodeContext) -> Result<(), DecodeError> {
        message::merge_repeated::<Self>(wire_type, values, buf, ctx)
    }
...
/// the same behaviour by providing the conversions required here alongside an
/// appropriate [`ProtoExt`] implementation.
