    ...
    fn write_markdown(&self) -> io::Result<()> {
        ...
        for (group_name, benchmarks) in groups.iter() {
            ...
            for (bench_name, aggregate) in benchmarks {
                ...
                let avg_ns = aggregate.total_duration.as_nanos() / aggregate.iterations as u128;
                let avg_seconds = avg_ns / 1_000_000_000;
                let avg_usecs = avg_ns / 1_000;
                let throughput = aggregate.bytes.map(|bytes| if avg_seconds > 0 { (bytes / MB) as u128 / avg_seconds } else { 0 });

                let throughput_display = throughput.map_or_else(|| "-".to_string(), |value| format!("{value:.2}"));

                writeln!(file, "| {bench_name} | {avg_ns:.2} | {avg_usecs:.2} | {throughput_display} |")?;
            }
...
fn bench_encode_decode(c: &mut Criterion) {
    ...
    //prost: 3778
    //proto: 4010
    //assert_eq!(prost_bytes.len(), proto_bytes.len());
