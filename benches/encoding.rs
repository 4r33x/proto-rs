...
impl BenchRecorder {
    ...
    fn write_markdown(&self) -> io::Result<()> {
        let groups = self.groups.lock().map_err(|_| io::Error::new(io::ErrorKind::Other, "bench recorder poisoned"))?;

        let base: PathBuf = env!("CARGO_MANIFEST_DIR").into();
        let path = base.join("benches").join("bench.md");

        let mut file = File::create(path)?;
        writeln!(file, "# Criterion Benchmark Summary\n")?;
        writeln!(file, "> Generated by `cargo bench --bench encoding`.\n")?;

        for (group_name, benchmarks) in groups.iter() {
            writeln!(file, "## {}\n", group_name)?;
            writeln!(file, "| Benchmark | Avg ns/op | Avg µs/op | MB/s |\n| --- | ---: | ---: | ---: |")?;

            for (bench_name, aggregate) in benchmarks.iter() {
                if aggregate.iterations == 0 {
                    continue;
                }

                let avg_seconds = aggregate.total_duration.as_secs_f64() / aggregate.iterations as f64;
                let avg_ns = avg_seconds * 1e9;
                let avg_us = avg_seconds * 1e6;
                let throughput = aggregate.bytes.map(|bytes| if avg_seconds > 0.0 { (bytes as f64 / 1_048_576.0) / avg_seconds } else { 0.0 });

                let throughput_display = throughput.map(|value| format!("{value:.2}")).unwrap_or_else(|| "-".to_string());

                writeln!(file, "| {} | {:.2} | {:.2} | {} |", bench_name, avg_ns, avg_us, throughput_display)?;
            }
...
fn bench_encode_decode(c: &mut Criterion) {
    ...
    group.bench_function("proto_rs encode_to_vec", |b| {
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("complex_root_encode_decode", "proto_rs encode_to_vec", total, iters, Some(proto_bytes.len() as u64));
    ...
    group.bench_function("prost encode", |b| {
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("complex_root_encode_decode", "prost encode", total, iters, Some(prost_bytes.len() as u64));
    ...
    group.bench_function("proto_rs decode", |b| {
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("complex_root_encode_decode", "proto_rs decode", total, iters, Some(proto_bytes.len() as u64));
    ...
    group.bench_function("prost decode", |b| {
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("complex_root_encode_decode", "prost decode", total, iters, Some(prost_bytes.len() as u64));
...
fn bench_zero_copy_vs_prost(c: &mut Criterion) {
    ...
    group.bench_function("prost clone + encode", |b| {
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("zero_copy_vs_prost", "prost clone + encode", total, iters, Some(proto_len as u64));
    ...
    group.bench_function("proto_rs zero_copy response", |b| {
        ...
        b.iter_custom(|iters| {
            ...
            bench_recorder().record("zero_copy_vs_prost", "proto_rs zero_copy response", total, iters, Some(proto_len as u64));
