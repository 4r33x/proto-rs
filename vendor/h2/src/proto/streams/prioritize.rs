...
mod owned_tests {
    use super::*;
    #[test]
    fn owned_write_prioritized_take_preserves_allocation() {
        let original = bytes::Bytes::from(vec![0x41; 65536]);
        let mut store = store::Store::new();
        let id = StreamId::from(1);
        let key = store
            .insert(id, stream::Stream::new(id, 65535, 65535))
            .key();
        let mut buf = Prioritized {
            inner: original.clone().take(16384),
            end_of_stream: false,
            stream: key,
        };
        let bytes = buf.copy_to_bytes(16384);
        assert_eq!(bytes.as_ptr(), original.as_ptr());
        assert_eq!(buf.remaining(), 0);
        assert_eq!(buf.inner.get_ref().remaining(), 65536 - 16384);
    }
}
