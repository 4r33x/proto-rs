...
impl Builder {
    ...
    where
        ...
        B: Buf,
    {
        Connection::handshake2(io, self.clone(), None)
    }

    /// Local proto-rs extension for ownership-aware transports.
    pub fn handshake_with_owned_write<T, B>(
        &self,
        io: T,
        writer: Option<crate::OwnedWrite<T>>,
    ) -> impl Future<Output = Result<(SendRequest<B>, Connection<T, B>), crate::Error>>
    where
        T: AsyncRead + AsyncWrite + Unpin,
        B: Buf,
    {
        Connection::handshake2(io, self.clone(), writer)
    }
}
