...
pub struct FramedWrite<T, B> {
    ...
    encoder: Encoder<B>,
    owned_write: Option<crate::OwnedWrite<T>>,
    pending_owned: Option<(Bytes, Bytes)>,
}
    ...
    B: Buf,
{
    ...
    pub fn flush(&mut self, cx: &mut Context) -> Poll<io::Result<()>> {
        ...
        loop {
            while !self.encoder.is_empty() || self.pending_owned.is_some() {
                if let Some(write) = self.owned_write {
                    if self.pending_owned.is_none() {
                        let position = self.encoder.buf.position() as usize;
                        let len = self.encoder.buf.get_ref().len();
                        let mut head = self.encoder.buf.get_mut().split_to(len).freeze();
                        head.advance(position);
                        ...
                        self.pending_owned = Some((head, data));
                    }
                    let (head, data) = self.pending_owned.as_mut().unwrap();
                    let n = ready!(write(Pin::new(&mut self.inner), cx, head, data))?;
                    if n == 0 || n > head.len() + data.len() {
                        return Poll::Ready(Err(io::Error::new(
                            io::ErrorKind::WriteZero,
                            "invalid owned write length",
                        )));
                    }
                    let header_bytes = n.min(head.len());
                    head.advance(header_bytes);
                    data.advance(n - header_bytes);
                    if head.is_empty() && data.is_empty() {
                        self.pending_owned = None;
                    }
                    continue;
                }
        ...
        // Owned header slices advance BytesMut's allocation window. Restore
        // staging capacity or poll_ready could return Pending forever after
        // enough frames, despite having nothing left to flush.
        if self.owned_write.is_some()
            && self.encoder.buf.get_ref().capacity() - self.encoder.buf.get_ref().len()
                < self.encoder.min_buffer_capacity
        {
            self.encoder.buf.get_mut().reserve(DEFAULT_BUFFER_CAPACITY);
        }
        // Flush the upstream
        ...
        match item {
            Frame::Data(mut v) => {
                ...
                if len >= self.chain_threshold {
                    ...
                    if !self.owned_output && self.buf.get_ref().remaining() < self.chain_threshold {
...
impl<T, B> FramedWrite<T, B> {
    pub(crate) fn set_owned_write(&mut self, writer: Option<crate::OwnedWrite<T>>) {
        self.encoder.owned_output = writer.is_some();
        self.owned_write = writer;
    }
...
mod owned_tests {
    ...
    impl AsyncWrite for Io {
        fn poll_write(
            ...
            panic!("borrowed write bypassed owned hook")
        }
    ...
    fn owned(
        ...
        io.pending = true;
        if !data.is_empty() {
            let start = io.original.as_ptr() as usize;
            assert!(
                (start..start + io.original.len()).contains(&(data.as_ptr() as usize)),
                "DATA payload was copied"
            );
        }
        let mut count = 0;
        for bytes in [head, data] {
            let n = bytes.len().min(17 - count);
            io.output.extend_from_slice(&bytes[..n]);
            count += n;
        }
        io.writes += 1;
        Poll::Ready(Ok(count))
    ...
    #[test]
    fn owned_write_preserves_payload_allocation_through_partial_writes_and_pending() {
        ...
        framed.set_owned_write(Some(owned));
        framed
            .buffer(frame::Data::new(frame::StreamId::from(1), payload.clone()).into())
            .unwrap();
        struct Noop;
        ...
        assert_eq!(framed.inner.output.len(), payload.len() + 9);
        assert_eq!(&framed.inner.output[9..], &payload[..]);
        assert!(framed.inner.writes > 1);
        assert_eq!(
            framed.take_last_data_frame().unwrap().payload().remaining(),
            0
        );
        // Exercise many successive frame headers, exhausting the original
        // header allocation window. Readiness must not park without a wakeup.
        for _ in 0..2048 {
