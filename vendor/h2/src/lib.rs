...
/// Local proto-rs extension: write immutable owned HTTP/2 header and DATA bytes.
/// The callback may retain clones after returning, but must not consume bytes on
/// Pending or Err. Success reports a prefix length across header then payload.
pub type OwnedWrite<T> = fn(
    std::pin::Pin<&mut T>,
