...
impl Connection {
    pub(crate) fn lazy_endpoint(endpoint: Endpoint) -> Self {
        match &endpoint.uri {
            super::super::endpoint::EndpointType::Uri(_) => Self::lazy(endpoint.http_connector(), endpoint),
            super::super::endpoint::EndpointType::Uds(path) => {
                let connector = endpoint.uds_connector(path);
                Self::lazy(connector, endpoint)
            }
        }
    }
        ...
        C::Response: rt::Read + rt::Write + Unpin + Send + 'static,
    {
        ...
        let make_service = MakeSendRequestService::new(connector, endpoint.executor.clone(), settings);
        Self::with_service(make_service, endpoint, is_lazy)
    }

    // Share the entire policy/reconnection stack with alternate HTTP/2 engines.
    pub(crate) fn with_service<M, S>(make_service: M, endpoint: Endpoint, is_lazy: bool) -> Self
    where
