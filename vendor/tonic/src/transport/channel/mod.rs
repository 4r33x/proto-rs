...
#[doc(hidden)]
pub use endpoint::OwnedHttp2Config;
use http::Request;
use http::Response;
...
use self::service::Connection;
use self::service::DynamicServiceStream;
use self::service::Executor;
use self::service::SharedExec;
use crate::body::Body;
...
impl Channel {
    /// Share Tonic's dynamic balancing/discovery with alternate connection
    /// factories. Returning None selects the normal endpoint transport (TLS).
    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub fn balance_channel_with_service<K, M, S, F>(capacity: usize, mut factory: F) -> (Self, Sender<Change<K, Endpoint>>)
    where
        K: Hash + Eq + Send + Clone + 'static,
        F: FnMut(&Endpoint) -> Option<M> + Send + 'static,
        M: Service<Uri, Response = S> + Send + 'static,
        M::Future: Unpin + Send,
        M::Error: Into<crate::BoxError> + Send,
        S: Service<Request<Body>, Response = Response<Body>> + Send + 'static,
        S::Future: Send,
        S::Error: Into<crate::BoxError> + Send,
    {
        let (tx, rx) = channel(capacity);
        let list = DynamicServiceStream::with_factory(rx, move |endpoint| match factory(&endpoint) {
            Some(service) => Connection::with_service(service, endpoint, true),
            None => Connection::lazy_endpoint(endpoint),
        });
        (Self::balance(list, DEFAULT_BUFFER_SIZE, SharedExec::tokio()), tx)
    }

    /// Install an alternate HTTP/2 connection factory without replacing Tonic's
    /// buffering, endpoint policies or reconnect semantics. The factory must
    /// apply endpoint TCP/HTTP2 settings; it must never replay consumed requests.
    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub async fn connect_with_service<M, S>(factory: M, endpoint: Endpoint) -> Result<Self, super::Error>
    where
        M: Service<Uri, Response = S> + Send + 'static,
        M::Future: Unpin + Send,
        M::Error: Into<crate::BoxError> + Send,
        ...
        S::Error: Into<crate::BoxError> + Send,
    {
        ...
        let svc = Connection::with_service(factory, endpoint, false).ready_oneshot().await.map_err(super::Error::from_source)?;
        let (svc, worker) = Buffer::pair(svc, buffer_size);
        executor.execute(worker);
        Ok(Self { svc })
    }

    /// Lazy counterpart of `connect_with_service`.
    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub fn new_with_service<M, S>(factory: M, endpoint: Endpoint) -> Self
    where
        M: Service<Uri, Response = S> + Send + 'static,
        M::Future: Unpin + Send,
        M::Error: Into<crate::BoxError> + Send,
        ...
        S::Error: Into<crate::BoxError> + Send,
    {
        let buffer_size = endpoint.buffer_size.unwrap_or(DEFAULT_BUFFER_SIZE);
        let executor = endpoint.executor.clone();
        let (svc, worker) = Buffer::pair(Connection::with_service(factory, endpoint, true), buffer_size);
        executor.execute(worker);
        Self { svc }
    }
    ...
    pub fn balance_list(list: impl Iterator<Item = Endpoint>) -> Self {
        ...
        list.for_each(|endpoint| {
            tx.try_send(Change::Insert(endpoint.uri.clone(), endpoint)).unwrap();
        });
        ...
        C::Response: rt::Read + rt::Write + Unpin + Send + 'static,
    {
        ...
        let svc = Connection::connect(connector, endpoint).await.map_err(super::Error::from_source)?;
        let (svc, worker) = Buffer::pair(svc, buffer_size);
        executor.execute(worker);
...
impl Future for ResponseFuture {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        Pin::new(&mut self.inner).poll(cx).map_err(super::Error::from_source)
    }
}
