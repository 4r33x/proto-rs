...
impl Endpoint {
    ...
    pub fn from_static(s: &'static str) -> Self {
        if s.starts_with("unix:") {
            let uds_filepath = s.strip_prefix("unix://").or_else(|| s.strip_prefix("unix:")).expect("Invalid unix domain socket URI");
    ...
    pub fn from_shared(s: impl Into<Bytes>) -> Result<Self, Error> {
        let s = str::from_utf8(&s.into()).map_err(|e| Error::new_invalid_uri().with(e))?.to_string();
        if s.starts_with("unix:") {
            let uds_filepath = s.strip_prefix("unix://").or_else(|| s.strip_prefix("unix:")).ok_or(Error::new_invalid_uri())?;
    ...
    pub fn tls_config(self, tls_config: ClientTlsConfig) -> Result<Self, Error> {
        match &self.uri {
            EndpointType::Uri(uri) => Ok(Endpoint {
                tls: Some(tls_config.into_tls_connector(uri).map_err(Error::from_source)?),
                ..self
            }),
    ...
    #[cfg(feature = "_tls-any")]
    pub fn tls_config_with_verifier(self, tls_config: ClientTlsConfig, verifier: Arc<dyn ServerCertVerifier>) -> Result<Self, Error> {
        match &self.uri {
            EndpointType::Uri(uri) => Ok(Endpoint {
                tls: Some(tls_config.into_tls_connector_with_verifier(uri, verifier).map_err(Error::from_source)?),
                ..self
            }),
            EndpointType::Uds(_) => Err(Error::new(error::Kind::InvalidTlsConfigForUds)),
        }
    ...
    pub(crate) fn http_connector(&self) -> service::Connector<HttpConnector> {
        self.connector(self.plain_http_connector())
    }

    fn plain_http_connector(&self) -> HttpConnector {
    ...
    pub async fn connect(&self) -> Result<Channel, Error> {
        match &self.uri {
            ...
            EndpointType::Uds(uds_filepath) => Channel::connect(self.uds_connector(uds_filepath.as_str()), self.clone()).await,
        }
    }
    ...
    pub fn connect_lazy(&self) -> Channel {
        match &self.uri {
            ...
            EndpointType::Uds(uds_filepath) => Channel::new(self.uds_connector(uds_filepath.as_str()), self.clone()),
        }
    }
    ...
    /// Why the proto-rs experimental owned backend must use this endpoint's
    /// normal transport instead. Never discard TLS or configured channel policy.
    #[doc(hidden)]
    pub fn owned_transport_fallback_reason(&self) -> Option<&'static str> {
        #[cfg(feature = "_tls-any")]
        if self.tls.is_some() {
            return Some("TLS requires the normal encrypted transport");
        }
        if self.uri().scheme_str() == Some("https") {
            return Some("TLS requires the normal encrypted transport");
        }
        if !matches!(&self.uri, EndpointType::Uri(uri) if uri.scheme_str() == Some("http")) {
            return Some("endpoint requires the normal Tonic connector");
        }
        None
    }

    /// Configured DNS/Happy-Eyeballs/TCP connector for the owned plaintext engine.
    /// Reject TLS/UDS before exposing a connector that cannot negotiate them.
    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub fn owned_http_connector(&self) -> Result<HttpConnector, crate::BoxError> {
        if let Some(reason) = self.owned_transport_fallback_reason() {
            return Err(std::io::Error::new(std::io::ErrorKind::InvalidInput, reason).into());
        }
        Ok(self.plain_http_connector())
    }

    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub fn execute_owned(&self, future: Pin<Box<dyn Future<Output = ()> + Send>>) {
        self.executor.execute(future);
    }

    #[doc(hidden)]
    #[cfg(feature = "proto-rs-owned")]
    pub fn owned_http2_config(&self) -> OwnedHttp2Config {
        OwnedHttp2Config {
            stream_window: self.init_stream_window_size,
            connection_window: self.init_connection_window_size,
...
/// HTTP/2 settings passed to the alternate engine; other policies stay in Tonic.
#[doc(hidden)]
#[cfg(feature = "proto-rs-owned")]
#[derive(Debug)]
pub struct OwnedHttp2Config {
