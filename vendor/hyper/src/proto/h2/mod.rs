...
impl<B: Buf> Buf for SendBuf<B> {
    fn copy_to_bytes(&mut self, len: usize) -> bytes::Bytes {
        match self {
            Self::Buf(b) => b.copy_to_bytes(len),
            Self::Cursor(c) => c.copy_to_bytes(len),
            Self::None => {
                assert_eq!(len, 0);
                bytes::Bytes::new()
            }
...
mod owned_buf_tests {
    use super::*;
    #[test]
    fn owned_write_sendbuf_preserves_bytes_allocation() {
        let original = bytes::Bytes::from(vec![0x41; 65536]);
        let mut buf = SendBuf::Buf(original.clone());
        for offset in (0..65536).step_by(16384) {
            let frame = buf.copy_to_bytes(16384);
            assert_eq!(frame.as_ptr(), original.slice(offset..).as_ptr());
        }
    }
}
