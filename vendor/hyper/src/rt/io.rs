...
pub trait Write {
    /// Local proto-rs extension: supports immutable owned header/DATA writes.
    /// A writer retaining bytes beyond this call must keep `Bytes` clones alive
    /// until its asynchronous consumers release them. Pending/Err accept no bytes.
    fn is_write_owned(&self) -> bool {
        false
    }

    /// Write a prefix across `head` followed by `data`, optionally retaining
    /// clones. The default follows the ordinary borrowed-write contract.
    fn poll_write_owned(
        self: Pin<&mut Self>,
        cx: &mut Context<'_>,
        head: &bytes::Bytes,
        ...
        self.poll_write_vectored(
            cx,
            &[std::io::IoSlice::new(head), std::io::IoSlice::new(data)],
        )
    }
...
macro_rules! deref_async_write {
    () => {
        ...
        fn poll_write_owned(
            ...
            Pin::new(&mut **self).poll_write_owned(cx, head, data)
        }
    ...
    P::Target: Write,
{
    ...
    fn poll_write_owned(
        ...
        pin_as_deref_mut(self).poll_write_owned(cx, head, data)
    }
