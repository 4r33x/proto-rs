...
fn response_to_proto_response(
    response_return_type: &Type,
    response_binding: &TokenStream,
    response_proto: &TokenStream,
) -> TokenStream {
...
fn wrap_call_future(is_async: bool, body: TokenStream) -> TokenStream {
    if is_async {
        ...
        wrap_async_block(quote! { ::core::future::ready(#body) }, true)
    }
}
...
fn generate_blanket_unary_method(method: &MethodInfo, trait_name: &syn::Ident) -> TokenStream {
    ...
    if method.is_async {
        ...
        let question = if method.response_is_result { quote! { ? } } else { quote! {} };
...
fn generate_unary_route_handler(method: &MethodInfo, route_path: &str, svc_name: &syn::Ident, trait_name: &syn::Ident) -> TokenStream {
    ...
    let call_future = wrap_call_future(method.is_async, quote! {
...
fn generate_streaming_route_handler(method: &MethodInfo, route_path: &str, svc_name: &syn::Ident, trait_name: &syn::Ident) -> TokenStream {
    ...
    let await_suffix = if method.is_async { quote! { .await } } else { quote! {} };
    let await_question_suffix = if method.is_async { quote! { .await? } } else { quote! { ? } };
