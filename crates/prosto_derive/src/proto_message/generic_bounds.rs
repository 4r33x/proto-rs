...
pub fn add_proto_wire_bounds<'a>(generics: &Generics, fields: impl IntoIterator<Item = &'a FieldInfo<'a>>) -> Generics {
    ...
    for ty in bound_types {
        where_clause.predicates.push(parse_quote!(#ty: ::proto_rs::ProtoEncode));
        where_clause.predicates.push(parse_quote!(#ty: ::proto_rs::ProtoDecode));
        where_clause.predicates.push(parse_quote!(#ty: ::proto_rs::ProtoExt));
    }
