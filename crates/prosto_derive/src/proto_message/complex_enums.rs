...
pub(super) fn generate_complex_enum_impl(
    ...
    // TODO: Complex enums need to be refactored for the new trait system
    return Err(syn::Error::new(
        input.ident.span(),
        "Complex enums (enums with data fields) are not yet supported with the new trait system. \
         Only simple enums (unit variants only) are currently supported. \
         Please convert your enum to use simple variants, or wait for complex enum support to be implemented."
    ));
