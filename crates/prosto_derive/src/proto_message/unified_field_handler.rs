...
// Removed: generate_proto_shadow_impl - no longer needed with new trait system
...
pub fn encode_input_binding(field: &FieldInfo<'_>, base: &TokenStream2) -> EncodeBinding {
    ...
    if needs_encode_conversion(&field.config, &field.parsed) {
        ...
        // Simple case: just use reference to the field
        EncodeBinding {
            prelude: None,
            value: quote! { &#access_expr },
        }
    }
}
...
// Removed old encoding/decoding helper functions - replaced by new ProtoArchive-based system
// The new system generates struct-based Archived types instead of using ProtoWire methods
