...
fn main() {
    ...
    // Test case #5: Verify custom wrapper type aliases don't produce erroneous generics
    // For example, CustomMutex<MEx> should generate "MEx" not "MEx<MEx>"
    // This tests the fix for the bug where generic_args were incorrectly appended to inner types
    assert!(
    ...
    assert!(
        client_contents.contains("pub arc: MEx,"),
        "arc field should be MEx, not MEx<MEx>"
    );
    assert!(
