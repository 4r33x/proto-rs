...
// ============================================================================
// GENERIC TYPES INTEGRATION TESTS
// ============================================================================
//
// NOTE: This test uses prost-generated types for all RPC communication.
// The proto files were generated by proto_rs with proto_generic_types, then
// compiled by prost. This demonstrates that proto_rs can generate proto files
// for generic types that interoperate correctly with prost-based RPC.
//
// Since proto_rpc requires ProtoExt trait (which prost types don't have),
// we use the prost-generated client/server directly for all tests.
// ============================================================================

use tonic_prost_test::generic_test;

// ============================================================================
// PROTO_RS SERVER IMPLEMENTATION (using prost traits)
// ============================================================================

struct GenericServiceImpl;

#[tonic::async_trait]
impl generic_test::generic_test_service_server::GenericTestService for GenericServiceImpl {
    async fn process_u64u32(
        &self,
        request: Request<generic_test::GenericRequestU64u32>,
    ) -> Result<Response<generic_test::GenericResponseU64u32>, Status> {
        let req = request.into_inner();
        let response = generic_test::GenericResponseU64u32 {
            result: req.key,
            data: req.value,
            success: req.count > 0,
        };
    ...
    async fn process_u64i32(
        &self,
        request: Request<generic_test::GenericRequestU64i32>,
    ) -> Result<Response<generic_test::GenericResponseU64i32>, Status> {
        let req = request.into_inner();
        let response = generic_test::GenericResponseU64i32 {
            result: req.key,
            data: req.value,
            success: req.count > 0,
        };
    ...
    async fn process_string_u32(
        &self,
        request: Request<generic_test::GenericRequestStringU32>,
    ) -> Result<Response<generic_test::GenericResponseStringU32>, Status> {
        let req = request.into_inner();
        let response = generic_test::GenericResponseStringU32 {
            result: req.key.clone(),
            data: req.value,
            success: req.count > 0,
        };
        Ok(Response::new(response))
    ...
    async fn process_string_i32(
        &self,
        request: Request<generic_test::GenericRequestStringI32>,
    ) -> Result<Response<generic_test::GenericResponseStringI32>, Status> {
        let req = request.into_inner();
        let response = generic_test::GenericResponseStringI32 {
            result: req.key.clone(),
            data: req.value,
            success: req.count > 0,
        };
...
// ============================================================================
// PROST SERVER IMPLEMENTATION
// ============================================================================

struct ProstGenericService;

#[tonic::async_trait]
impl tonic_prost_test::generic_test::generic_test_service_server::GenericTestService for ProstGenericService {
    async fn process_u64u32(
        &self,
        request: Request<tonic_prost_test::generic_test::GenericRequestU64u32>,
    ) -> Result<Response<tonic_prost_test::generic_test::GenericResponseU64u32>, Status> {
        let req = request.into_inner();
        let response = tonic_prost_test::generic_test::GenericResponseU64u32 {
            result: req.key,
            data: req.value,
            success: req.count > 0,
        };
    ...
    async fn process_u64i32(
        &self,
        request: Request<tonic_prost_test::generic_test::GenericRequestU64i32>,
    ) -> Result<Response<tonic_prost_test::generic_test::GenericResponseU64i32>, Status> {
        let req = request.into_inner();
        let response = tonic_prost_test::generic_test::GenericResponseU64i32 {
            result: req.key,
            data: req.value,
            success: req.count > 0,
        };
    ...
    async fn process_string_u32(
        &self,
        request: Request<tonic_prost_test::generic_test::GenericRequestStringU32>,
    ) -> Result<Response<tonic_prost_test::generic_test::GenericResponseStringU32>, Status> {
        let req = request.into_inner();
        let response = tonic_prost_test::generic_test::GenericResponseStringU32 {
            result: req.key.clone(),
            data: req.value,
            success: req.count > 0,
        };
    ...
    async fn process_string_i32(
        &self,
        request: Request<tonic_prost_test::generic_test::GenericRequestStringI32>,
    ) -> Result<Response<tonic_prost_test::generic_test::GenericResponseStringI32>, Status> {
        let req = request.into_inner();
        let response = tonic_prost_test::generic_test::GenericResponseStringI32 {
            result: req.key.clone(),
            data: req.value,
            success: req.count > 0,
        };
...
async fn spawn_generic_proto_server() -> (
    std::net::SocketAddr,
    ...
    let handle = tokio::spawn(async move {
        Server::builder()
            .add_service(
                generic_test::generic_test_service_server::GenericTestServiceServer::new(
                    GenericServiceImpl,
                ),
            )
            .serve_with_incoming_shutdown(incoming, async {
...
async fn spawn_generic_prost_server() -> (
    std::net::SocketAddr,
    tokio::sync::oneshot::Sender<()>,
    tokio::task::JoinHandle<Result<(), tonic::transport::Error>>,
) {
    ...
    let handle = tokio::spawn(async move {
        Server::builder()
            .add_service(
                tonic_prost_test::generic_test::generic_test_service_server::GenericTestServiceServer::new(
                    ProstGenericService,
                ),
            )
            .serve_with_incoming_shutdown(incoming, async {
...
#[tokio::test(flavor = "multi_thread")]
async fn generic_proto_client_against_proto_server() {
    // Test: prost client <-> proto_rs implemented server
    // This demonstrates that proto_rs can implement servers using prost-generated types
    // from proto files that proto_rs generated via proto_generic_types
    let (addr, shutdown, handle) = spawn_generic_proto_server().await;

    let mut client =
        generic_test::generic_test_service_client::GenericTestServiceClient::connect(
            format!("http://{addr}")
        )
        .await
        .unwrap();
    ...
    // Test u64, u32
    let request = generic_test::GenericRequestU64u32 {
        key: 42,
        value: 100,
        count: 5,
    };
    let response = client
        .process_u64u32(tonic::Request::new(request))
        .await
        .unwrap()
        .into_inner();
    ...
    // Test u64, i32
    let request = generic_test::GenericRequestU64i32 {
        key: 99,
        value: -50,
        count: 0,
    };
    let response = client
        .process_u64i32(tonic::Request::new(request))
        .await
        .unwrap()
        .into_inner();
    assert_eq!(response.result, 99);
    assert_eq!(response.data, -50);
    assert_eq!(response.success, false);

    // Test String, u32
    let request = generic_test::GenericRequestStringU32 {
        key: "hello".to_string(),
        value: 200,
        count: 10,
    };
    let response = client
        .process_string_u32(tonic::Request::new(request))
        .await
        .unwrap()
        .into_inner();
    ...
    // Test String, i32
    let request = generic_test::GenericRequestStringI32 {
        key: "world".to_string(),
        value: -100,
        count: -1,
    };
    let response = client
        .process_string_i32(tonic::Request::new(request))
        .await
        .unwrap()
        .into_inner();
    assert_eq!(response.result, "world");
    assert_eq!(response.data, -100);
    assert_eq!(response.success, false);
...
#[tokio::test(flavor = "multi_thread")]
async fn prost_client_against_proto_server() {
    // Test: prost generated client <-> proto_rs generated server
    // Both use the same prost types, demonstrating full interoperability
    let (addr, shutdown, handle) = spawn_generic_proto_server().await;

    let mut client =
        tonic_prost_test::generic_test::generic_test_service_client::GenericTestServiceClient::connect(
            format!("http://{addr}")
        )
        .await
        .unwrap();
    ...
    // Test u64, u32
    let request = tonic_prost_test::generic_test::GenericRequestU64u32 {
        key: 42,
        value: 100,
        count: 5,
    };
    ...
    // Test u64, i32
    let request = tonic_prost_test::generic_test::GenericRequestU64i32 {
        key: 99,
        value: -50,
        count: 0,
    };
    ...
    // Test String, u32
    let request = tonic_prost_test::generic_test::GenericRequestStringU32 {
        key: "hello".to_string(),
        value: 200,
        count: 10,
    };
    ...
    // Test String, i32
    let request = tonic_prost_test::generic_test::GenericRequestStringI32 {
        key: "world".to_string(),
        value: -100,
        count: -1,
    };
...
#[tokio::test(flavor = "multi_thread")]
async fn proto_client_against_prost_server() {
    // Test: proto_rs generated client <-> prost server
    // Both use the same prost types, demonstrating full interoperability
    let (addr, shutdown, handle) = spawn_generic_prost_server().await;
