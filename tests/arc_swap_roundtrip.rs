...
fn arc_swap_roundtrip_preserves_inner_value() {
    ...
    let encoded = <SwapHolder as ProtoEncode>::encode_to_vec(&holder);
...
fn arc_swap_option_roundtrip_handles_present_value() {
    ...
    let encoded = <OptionalSwapHolder as ProtoEncode>::encode_to_vec(&holder);
    let decoded = <OptionalSwapHolder as ProtoDecode>::decode(&encoded[..], DecodeContext::default()).expect("decode arc swap option");
...
fn arc_swap_option_roundtrip_handles_absent_value() {
    ...
    let encoded = <OptionalSwapHolder as ProtoEncode>::encode_to_vec(&holder);
    let decoded =
        <OptionalSwapHolder as ProtoDecode>::decode(&encoded[..], DecodeContext::default()).expect("decode default arc swap option");
...
fn arc_swap_vec_u8_roundtrip_encodes_as_bytes() {
    ...
    let encoded = <ArcSwapContainerHolder as ProtoEncode>::encode_to_vec(&holder);
...
fn arc_swap_option_bytes_roundtrip_handles_presence_and_absence() {
    ...
    let encoded = <ArcSwapOptionBytesHolder as ProtoEncode>::encode_to_vec(&holder);
    let decoded =
        <ArcSwapOptionBytesHolder as ProtoDecode>::decode(&encoded[..], DecodeContext::default()).expect("decode arc swap option bytes");
    ...
    let encoded_default = <ArcSwapOptionBytesHolder as ProtoEncode>::encode_to_vec(&default_holder);
