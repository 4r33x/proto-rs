...
fn collections_roundtrip() {
    ...
    let decoded =
        <CollectionsMessage as ProtoDecode>::decode(bytes.clone(), DecodeContext::default()).expect("decode collections message");
...
fn assert_decode_roundtrip(bytes: Bytes, proto_expected: &SampleMessage, prost_expected: &SampleMessageProst) {
    let decoded_proto =
        <SampleMessage as ProtoDecode>::decode(bytes.clone(), DecodeContext::default()).expect("proto decode failed");
...
{
    let shadow = <T::Shadow<'_> as ProtoShadowEncode<'_, T>>::from_sun(value);
    let archived = proto_rs::ArchivedProtoInner::<TAG, T::Shadow<'_>>::new(&shadow);
    archived.encode(buf);
}
...
fn proto_and_prost_encodings_are_equivalent() {
    ...
    let proto_decoded_from_prost =
        <SampleMessage as ProtoDecode>::decode(prost_bytes.clone(), DecodeContext::default()).expect("proto decode from prost bytes failed");
...
fn try_from_fn_decode_error() {
    ...
    let prost_bytes = encode_prost_message(&prost_msg);
    let err =
        <MixedProto as ProtoDecode>::decode(prost_bytes, DecodeContext::default()).expect_err("decoding should fail due to conversion error");
...
fn length_delimited_round_trips() {
    ...
    let decoded_proto = decode_length_delimited::<SampleMessage>(proto_bytes.clone())
        .expect("proto length-delimited decode failed");
    assert_eq!(decoded_proto, proto_msg);

    let decoded_proto_from_prost = decode_length_delimited::<SampleMessage>(prost_bytes.clone())
...
fn mixed_proto_cross_roundtrip_with_prost() {
    ...
    let decoded_proto_from_prost =
        <MixedProto as ProtoDecode>::decode(prost_bytes.clone(), DecodeContext::default()).expect("mixed proto decode from prost bytes failed");
    assert_eq!(decoded_proto_from_prost, proto_msg);
...
fn zero_copy_container_roundtrip() {
    ...
    let decoded =
        <ZeroCopyContainer as ProtoDecode>::decode(Bytes::from(encoded), DecodeContext::default()).expect("decode fixture");
